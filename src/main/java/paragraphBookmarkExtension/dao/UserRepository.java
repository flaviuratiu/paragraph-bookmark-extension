...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Transactional
    @Modifying
    @Query("UPDATE User user SET user.bookmarksEmailReminder = :enabled WHERE user.id = :userId")
    void updateBookmarkEmailReminderFlag(@Param("userId") long userId, @Param("enabled") boolean enabled);
